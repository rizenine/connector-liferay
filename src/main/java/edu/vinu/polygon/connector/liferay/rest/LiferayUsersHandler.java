...
public class LiferayUsersHandler {
    ...
    public String getCompanyUsersCount(HttpEntityEnclosingRequestBase request, CloseableHttpClient client) throws IOException, URISyntaxException {
    ...
    public CloseableHttpResponse loadCompanyUsers(ResultsHandler handler, HttpEntityEnclosingRequestBase request, CloseableHttpClient client) throws IOException, URISyntaxException {
      request.setEntity(new StringEntity(String.format(companyUsersEntity, config.getCompanyId(), 0, getCompanyUsersCount(request, client))));
      CloseableHttpResponse response = client.execute(request);
    ...
    public Uid addUser(ObjectClass oc, Set<Attribute> attrs, OperationOptions oo, HttpEntityEnclosingRequestBase request, CloseableHttpClient client) throws IOException, UnsupportedEncodingException {
      request.setEntity(addUserEntity(attrs));
      LOG.ok(">>> addUser Attribute {0}", attrs);
      LOG.ok(">>> addUser request(URI) {0}", request.getURI());

      CloseableHttpResponse response = client.execute(request);

      String result = EntityUtils.toString(response.getEntity(), "UTF-8");
      LOG.ok(">>> addUser response {0}", response);
      LOG.ok(">>> addUser response body {0}", result);
      JSONObject jors = new JSONObject(result);
      LOG.ok(">>> addUser response json {0}", jors);
    ...
    private HttpEntity addUserEntity(Set<Attribute> attrs) throws UnsupportedEncodingException{
      ...
      users.put("password1", "tetst123");
      users.put("password2", "tetst123");
    ...
    public void deleteUser(ObjectClass oc, Uid uid, OperationOptions oo, HttpEntityEnclosingRequestBase request, CloseableHttpClient client) throws IOException, UnsupportedEncodingException {
      ...
      LOG.ok(">>> deleteUser JSON {0}", root.toString());

      request.setEntity(new StringEntity(root.toString(), "UTF-8"));
      LOG.ok(">>> deleteUser request(URI) {0}", request.getURI());
